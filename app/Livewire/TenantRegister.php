...
class TenantRegister extends Component
{
    ...
    {
        $this->form->package = request()->query('package'); // check existance in backend
        if (Package::where("name", $this->form->package)->exists()) {
            $this->form->currentPackage = Package::where("name", $this->form->package)->first();
        }else{
            $this->form->currentPackage = new Package();
        }
    }
